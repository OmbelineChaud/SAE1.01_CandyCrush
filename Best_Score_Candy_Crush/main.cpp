...
//pour enregistrer les meilleures scores
struct bestScore {
    string utilisateur;
    int score;
};
...
//lire le fichier des meilleures scores
//l'utilisateur
string litUneString (){
    string uneChaine;
    while (true){
...
//comparer l'utilisateur et le score avec celui qui vient d'être fait.
void meilleurScore(string &user, int &score){
    string pastUser = litUneString();
    int otherScore = litUnEntier();
    if (user == pastUser && score == otherScore){
        cout << "Vous et votre score sont déjà enregistré";
    }
    else if (user == pastUser && score < otherScore){
        cout << "Vous avez fait mieux avant, votre score actuel ne sera pas enregistré.";
    }
    else if (user == pastUser){
        //la le score devrait forcément être supérieure.
    }
}
...
int main() {
    ...
    cout << "Entrez le nom sous lequel vous souhaitez l'enregistrer!" << endl;
    string utilisateur;
    cin >> utilisateur;
    meilleurScore(utilisateur, score);
    couleur(KReset);
    return 0;
}
